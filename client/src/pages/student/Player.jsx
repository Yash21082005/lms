...
const Player = () => {
  ...
  const getCourseData = () => {
    ...
    if (foundCourse) setCourseData(foundCourse); course.courseRatings.map(()=>{
  ...
  const markLectureAAsCompleted = async()=>{
    try {
          ...
          {playerData ? (
            <div>
              ...
              <div className='flex justify-between items-center mt-1'>
                ...
                <button onClick={()=>markLectureAAsCompleted(playerData.lectureId)} className='text-blue-600'>{progressData && progressData.lectureCompleted.includes(playerData.lectureId) ? 'Completed' : 'Mark Complete'}</button>
              </div>
            </div>
          ) :
